...
type CarouselAction = CarouselJumpAction | CarouselNextAction | CarouselPrevAction | CarouselDragAction | CarouselDoneAction;
...
export function useCarousel(length: number, interval: number): [number, (n: number) => void, SwipeableHandlers, React.CSSProperties] {
    ...
    useEffect(() => {
        const id = setTimeout(() => dispatch({ type: 'done' }), 400);
        return () => clearTimeout(id);
    }, [state.desired]);
